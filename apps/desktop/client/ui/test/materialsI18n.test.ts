...
const SHELL_KEYS = [
  ...
  // E10.4 W4：在线导入弹窗（OnlineImportDialog 消费面）。
  'online.open',
  'online.title',
  'online.tabUrl',
  'online.tabSearch',
  'online.scopeLabel',
  'online.urlLabel',
  'online.urlPlaceholder',
  'online.categoryLabel',
  'online.categoryHint',
  ...(['community-wiki', 'criticism', 'author-interview', 'other'] as const).map((c) => `online.category.${c}`),
  'online.import',
  'online.importing',
  'online.close',
  'online.queryPlaceholder',
  'online.search',
  'online.searching',
  'online.searchFailed',
  'online.searchEmpty',
  'online.importChecked',
  'online.batchOverflow',
  'online.needProject',
  'online.sourceWeb',
  'online.sourceWiki',
  'online.resultTitle',
  ...(['registered', 'reused', 'orphaned'] as const).map((o) => `online.outcome.${o}`),
  'online.resultTruncated',
  'online.resultFailed',
  // 失败分类八档 = 契约六档（MATERIAL_ONLINE_IMPORT_FAILURE_KINDS）+ invalid-input/unregistered
  //（MaterialsImportOnlineResult 失败联合全量——UI 分文案照材料页 rejectedKind 形态）。
  ...(
    [
      'bad-url', 'fetch-failed', 'empty-content', 'oversize',
      'stem-conflict', 'ingest-failed', 'invalid-input', 'unregistered',
    ] as const
  ).map((k) => `online.failureKind.${k}`),
  'provenance.title',
...
describe('materials 页 i18n 齐平（zh/en）', () => {
  ...
  it('插值占位完整（两侧——丢占位 = 渲染裸模板）', () => {
    const placeholders: Array<[string, string]> = [
      ...
      ['online.categoryHint', '{medium}'],
      ['online.categoryHint', '{tier}'],
      ['online.searchFailed', '{message}'],
      ['online.importChecked', '{count}'],
      ['online.batchOverflow', '{max}'],
