...
const SHELL_KEYS = [
  ...
  ...DECON_DIMENSION_IDS.map((d) => `dimGranularity.${d}`),
  ...DECON_JOB_STATUSES.map((s) => `status.${s}`),
  ...(['pending', 'running', 'gate-paused', 'paused', 'capped', 'stale', 'failed', 'done', 'cancelled', 'openReading'] as const).map(
    (k) => `banner.${k}`,
  ),
  ...(['p1a', 'p1b', 'p1c', 'p2', 'p3a', 'p3b', 'p4', 'p5', 'p6'] as const).map((p) => `pass.${p}`),
  ...(['materialChapter', 'arc', 'scene', 'all', 'arcs', 'stats', 'literal'] as const).map((u) => `unit.${u}`),
  ...(['total', 'eta', 'unitOfTotal', 'passCoverage'] as const).map((k) => `progress.${k}`),
  ...([
    'title', 'material', 'materialPlaceholder', 'materialEmpty', 'materialExcludedHint', 'tier',
    'dimensions', 'dimFineCount', 'dimCoarseHint', 'dimDeepHint', 'dimCoarseMore', 'dimPreviewShow',
    'dimPreviewHide', 'invalidDims', 'reviewGates',
  ...
  // CR-17：预估行通俗说明（pass slug 键——deconEstimateNoteKey 路由的穷举面）。
  ...(['p1a', 'p1b', 'p1c', 'p2', 'p3a', 'p4', 'p4style', 'p5book', 'p5chapter', 'p5scene', 'p6'] as const).map(
    (p) => `wizard.estimateNote.${p}`,
  ),
  ...
  ...([
    ...
    'chaptersEmpty', 'scenesEmpty', 'styleEmpty', 'styleStatsTitle', 'styleSentenceLabel',
    'styleParagraphLabel', 'styleDialogueLabel', 'styleDistribution', 'styleDialogueRatio',
    'styleNeedsFine', 'canonEmpty', 'selectReport', 'selectChapter', 'selectScene',
    'coarseLockedHint', 'styleExport', 'styleExportNoProject', 'styleExportConfirmTitle',
    'styleExportConfirmBody', 'styleExportDone', 'styleExportFailed', 'craftZoneTitle',
    'craftZoneCount', 'craftZoneEmpty', 'craftZoneJump', 'reportFailed', 'anchorsTitle',
    'anchorRow',
  ...
  ...([
    'aiView', 'evidenceExact', 'evidenceInferred', 'anchorsCount', 'provenance', 'summary',
    'statement', 'baseline', 'identity', 'speechPattern', 'traits', 'traitImmutable',
    'traitEvolvable', 'abilities', 'arc', 'coreTrauma', 'pillars', 'voiceAnchors', 'antiVoice',
    'neverDo', 'aliases', 'mentions', 'portraitDegraded', 'consistency', 'consistencyExact',
    'consistencyLoose', 'consistencyConflict', 'events', 'eventRow', 'eventUnannotated',
    'conflictNote', 'pair', 'edgeCount', 'edgeCountValue',
  ] as const).map((k) => `canonCard.${k}`),
  ...(['refresh', 'pause', 'resume', 'resumeDecon', 'resumeFromChapter', 'resumeWithBudget', 'cancel', 'delete', 'confirmRerun'] as const).map(
    (a) => `action.${a}`,
  ),
  'deleteConfirmTitle',
...
describe('decon 页 i18n 齐平（zh/en）', () => {
  ...
  it('插值占位完整（两侧——丢占位 = 渲染裸模板）', () => {
    const placeholders: Array<[string, string]> = [
      ...
      ['progress.eta', '{eta}'],
      ['progress.unitOfTotal', '{unit}'],
      ['progress.unitOfTotal', '{total}'],
      ['progress.passCoverage', '{done}'],
      ['progress.passCoverage', '{total}'],
      ['action.resumeFromChapter', '{chapter}'],
      ['canonCard.anchorsCount', '{count}'],
      ['canonCard.provenance', '{book}'],
      ['canonCard.events', '{count}'],
      ['canonCard.eventRow', '{chapter}'],
      ['canonCard.eventRow', '{label}'],
      ['canonCard.eventUnannotated', '{chapter}'],
      ['canonCard.edgeCountValue', '{count}'],
      ['wizard.materialExcludedHint', '{count}'],
      ['wizard.dimFineCount', '{count}'],
      ['wizard.dimCoarseMore', '{count}'],
      ...
      ['output.styleDistribution', '{count}'],
      ['output.styleDistribution', '{avg}'],
      ['output.styleDistribution', '{max}'],
      ['output.styleDialogueRatio', '{ratio}'],
